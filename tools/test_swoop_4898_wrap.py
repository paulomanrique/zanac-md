...
def main() -> int:
    ...
    # Leftover same-class CALLs (one-PR: do not ship).
    tracker = fn_span(ent, "static void tracker_step(Slot *e)")
    if not tracker:
        fail("tracker_step not found")
        fails += 1
    elif "ypos +=" not in tracker:
        fail("one-PR: tracker leftover signed s32 should remain")
        fails += 1
    else:
        print("  leftover: tracker_step still signed s32 (7f7b, not this PR)")
