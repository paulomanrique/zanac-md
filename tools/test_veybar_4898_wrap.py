...
def main() -> int:
    ...
    if asm:
        ...
        if len(swoop) < 2:
            ...
            if not re.search(r"CALL\s+0x4898\s*;\s*0x7e55", sbody, re.I):
                fail("swoop leftover CALL 4898 at 7e55 missing from ASM")
                fails += 1
            else:
                print("  ASM 7e55: swoop CALL 4898 (leftover, not this PR)")
    else:
    ...
    if not swoop:
        ...
        fails += 1
    elif "step_88_4898" in swoop or "step_88_y_4898" in swoop:
        fail("one-PR: do not ship swoop 7e55 in this change")
        fails += 1
    elif "ypos +=" not in swoop:
        fail("swoop leftover signed s32 should remain until its own PR")
        fails += 1
    else:
        print("  leftover: swoop_step still signed s32 (7e55, not this PR)")
